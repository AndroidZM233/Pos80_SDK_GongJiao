...
public class JTBCardManager implements ICardInterface {
    ...
    @Override
    public CardBackBean mainMethod(BankCard mBankCard, byte[] cpuCard, byte[] lPurSub) {
    ...
    @Override
    public int getFirst(BankCard mBankCard) {
        ...
        } else if (Arrays.equals(resultBytes, CardMethods.APDU_RESULT_FAILE)) {
...
//            OnAppendRecord(0xE0);
    ...
    public CardBackBean consumption(BankCard mBankCard, byte[] cpuCard) {
        ...
        if (tCardOpDU.ucCAPP == 1) {
            ...
            LogUtils.d("===更新1E文件(80dc)send===" + update80Dc);
            resultBytes = CardMethods.sendApdus(mBankCard, BankCard.CARD_MODE_PICC
                    , Datautils.HexString2Bytes(update80Dc));
            if (resultBytes == null || resultBytes.length == 2) {
