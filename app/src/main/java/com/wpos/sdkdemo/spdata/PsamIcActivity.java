...
import com.wpos.sdkdemo.DataConversionUtils;
import com.wpos.sdkdemo.R;
import com.wpos.sdkdemo.spdata.been.IcCardBeen;
import com.wpos.sdkdemo.spdata.been.TCommInfo;
import com.wpos.sdkdemo.spdata.been.PsamBeen;
import com.wpos.sdkdemo.spdata.utils.DataUtils;
import com.wpos.sdkdemo.util.TLV;
...
public class PsamIcActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private int isExpense = 0;//消费返回流程错误标识
...
//            //测试验证微智写接口
//            byte[] gg = lodkey[3];
//            retvalue = mBankCard.m1CardKeyAuth(0x41, 12, gg.length, gg, snUid.length, snUid);
//            retvalue = mBankCard.m1CardReadBlockData(12, respdata, resplen);
//            if (retvalue != 0) {
//                Log.e(TAG, "M1ICCard: 读失败");
//                return false;
//            }
//            byte[] test = cutBytes(respdata, 1, resplen[0] - 1);
//            Log.e(TAG, "M1ICCard:" + retvalue + "     写块 " + HEX.bytesToHex(test));
//            retvalue = mBankCard.m1CardWriteBlockData(10, test.length, test);
//            if (retvalue != 0) {
//                Log.e(TAG, "M1ICCard: 写失败");
//                return false;
//            }
//            retvalue = mBankCard.m1CardReadBlockData(10, respdata, resplen);
//            byte[] test2 = cutBytes(respdata, 1, resplen[0] - 1);
//            if (retvalue != 0) {
//                Log.e(TAG, "M1ICCard: 读失败");
//                return false;
//            }
//            Log.e(TAG, "M1ICCard:" + retvalue + "    读块" + HEX.bytesToHex(test2));
//            //测试完成
...
//                    retvalue = mBankCard.m1CardWriteBlockData(10, bytes09.length, bytes09);
                    if (retvalue != 0) {
                        Log.e(TAG, "M1ICCard: 写2区10块失败");
                        isExpense = 1;
                        return false;
                    }
                }
...
//            }
                ...
                retvalue = mBankCard.m1CardValueOperation(0x2D, 9, 1, 9);
...
//                retvalue = mBankCard.m1CardWriteBlockData(10, dtZ.length, dtZ);
                if (retvalue != 0) {
